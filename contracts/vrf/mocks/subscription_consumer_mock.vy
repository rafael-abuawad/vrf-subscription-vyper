...
def requestRandomWords(
    _enable_native_payment: bool
):
    """
    @dev Requests random words from the VRF coordinator.
    @notice This function is used to request random words from the VRF coordinator.
    @param _enable_native_payment The Boolean flag that determines
    """
    subscription_consumer._request_random_words(_enable_native_payment)
...
def fulfillRandomWords(
    ...
    @notice IMPORTANT: This function must be implemented by the contract
            that is calling the VRF coordinator.
...
def rawFulfillRandomWords(
    ...
    @notice In VRF v2.5, the coordinator calls fulfillRandomWords directly.
            ...
            IMPORTANT: This function must be implemented by the contract
            that is calling the VRF coordinator if is going to be used in the future.
    @param _request_id The 32-byte unique identifier for the
           randomness request that is being fulfilled.
    @param _random_words The array of random words generated by
           the VRF coordinator. The length of this array must
